...
object Codec {
  ...
  def decrypt(encrypted: String) : String =
    scala.util.Try( new String( dciphers.get().doFinal(
  ...
  // - Private -------------------------------
  // Parameters
  val AES_KEY:Array[Byte] = Array(244, 53, 89, 109, 11,
    48, 249, 235, 200, 47, 17, 2, 82, 22, 142, 87, 29, 139, 194, 124, 44,
    209, 142, 132, 240, 166, 226, 1, 157, 109, 53, 142).map( _.toByte )
  val AES_IV:Array[Byte] = Array(192, 65, 29, 85, 193,
    29, 62, 25, 69, 95, 16, 221, 149, 29, 128, 224).map( _.toByte )
